...
# Constants
data_dir = '/data/abhidip/QG_data/CU-stroy-QA-Data/'
...
def format_decoder_text_for_bart(example, task = "ask_question"):
    ...
    if task == "ask_question":
        decoder_text = "<s>{}: answer: {}".format(attribute, example["answer"])
    elif task == "ask_answer":
        decoder_text = "<s>{}: question: {}".format(attribute, example["question"])
    else:
        decoder_text = "<s>{}: ".format(attribute)
...
class fairytale_dataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        for b in batch:
            ...
            decoder_attention_masks.append(b['decoder_attention_mask'])
            if self.istraining:
...
if __name__ == '__main__':
    tokenizer = BartTokenizer.from_pretrained("/data/abhidip/QG_data/experiments/model/checkpoint-20-15750/")
